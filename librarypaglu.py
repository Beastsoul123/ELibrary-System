...
def addbook():
    ...
    b8.place(x=220,y=5)
    win.mainloop()
#addbook ka button kon add karega tera baap??
def addbooks():
    connectdb()
    global cur,con
...
def returnbooks():
    ...
    if i<=e:
        ...
        messagebox.showinfo("Return",' Book Returned with ')
        con.commit()
    win.destroy()
    closedb()
    libr()
...
def adminbooks():
    ...
    qq="SELECT * FROM book WHERE serial = '%i'"
    if e1.get()=='' or e2.get()=='' or e3.get()=='' or e4.get()=='':
        messagebox.showerror("Book","Show all Details")
        return None
    cur.execute(qq%(int(e4.get())))
    iddd = cur.fetchone()
    if iddd is None:
...
def home():
    try:
        ...
        usid=Label(window,text='USER ID', font='cosmicsansms 10 italic', bg='darkblue')
        paswrd=Label(window,text='PASSWORD', font='cosmicsansms 10 italic', bg='darkblue')
        wel.pack(fill=X)
        lib.pack(fill=X)
